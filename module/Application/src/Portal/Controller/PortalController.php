...
class PortalController extends AbstractActionController
{
    ...
    private function buildView()
    {
        ...
        $this->layout()->setVariable('layoutVariables', [
            ...
            'env' => $this->config['env'],
            'userType' => !empty($sessionDetails['userType']['userTypeName']) && $this->action === 'dashboard'
    ...
    private function process($post, $files)
    {
        ...
        switch ($post['process']) {
            ...
            case 'update-permit':
                ...
                break;
            case 'get-report':
                $process = $this->dashboardService->getReport($post);
                $viewOptions['activeTab'] = 'report';
                $viewOptions['report'] = $process['details'];
                break;
        }
